...
<template>
    <el-form ref="office-form" label-width="120px" label-position="right" style="width:370px;margin-top:20px; border: 1px solid gray">
...
export default {
    ...
    handleSave() {
      if (this.onSave) {
        if (this.color) {
          ...
          this.currentGoods.sizes[0].colors = [
            {
              id: this.color.id,
  ...
  mounted() {
    this.currentGoods.sizes[0].colors[this.index].name = this.color.name
    this.currentGoods.sizes[0].colors[this.index].count = this.color.count
